...
export class ProductsComponent implements OnInit {
  ...
  handleAction(element: any = '', action): void {
    if (action === 'add') {
      ...
      this.route.navigate(['/dashboard/product/product-detail'], {queryParams: {_id: element.id}});
    } else if (action === 'edit') {
      this.route.navigate(['/dashboard/product/product-add'], {queryParams: {_id: element.id, action: 'edit'}});
